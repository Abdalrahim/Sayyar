...
struct NewPlaceView : View {
    ...
    var body : some View {
        VStack(alignment: .center, spacing: 0) {
            ...
            Text(self.place.location)
            .font(.custom("Cairo-Regular", size: 10))
                .foregroundColor(.gray).hidden()
            
        }.frame(width: 75 , height: 75)
            .padding(5)
            .overlay(
                RoundedRectangle(cornerRadius: 10).stroke(Color(#colorLiteral(red: 0.6666666667, green: 0.6235294118, blue: 0.7215686275, alpha: 1)), lineWidth: 1)
        )
    }
}
...
struct PlaceView_Previews: PreviewProvider {
    static var previews: some View {
        PlaceView(place: FavPlace(name: "home", pType: .home, location: "10mins"), selected: true)
    }
}
